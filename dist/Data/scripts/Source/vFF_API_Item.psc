Scriptname vFF_API_Item extends vFF_APIBase Hidden
...
String Function AssignItemID(Int ajObjectInfo) Global
	...
	;Debug.Trace("vFF/API/Item/AssignItemID: Attempting to match item with form " + kForm + "...")
	...
	While i > 0
		...
		If kForm as Weapon || kForm as Armor
			...
			;Debug.Trace("vFF/API/Item/AssignItemID: " + kForm + " is a weapon or armor!")
			;Debug.Trace("vFF/API/Item/AssignItemID: " + kForm + " .Enchantment.Effects[0].MagicEffect is " + JValue.SolveForm(ajObjectInfo,".Enchantment.Effects[0].MagicEffect"))
			;Debug.Trace("vFF/API/Item/AssignItemID: Saved form's .Enchantment.Effects[0].MagicEffect is " + JValue.SolveForm(jItemInfo,".Enchantment.Effects[0].MagicEffect"))
		...
		ElseIf kForm as Potion
			Debug.Trace("vFF/API/Item/AssignItemID: " + kForm + " is a potion!")
			...
			EndIf
		Else
			Debug.Trace("vFF/API/Item/AssignItemID: " + kForm + " is something I don't know how to check!")
		EndIf
	EndWhile
...
String Function SerializeEquipment(ObjectReference akObject) Global
	...
	If kItemEnchantment
		...
		;Debug.Trace("vFF/CM: " + kItem.GetName() + " has enchantment " + kItemEnchantment.GetFormID() + ", " + kItemEnchantment.GetName())
...
;		AddToReqList(kItemEnchantment,"Enchantment")
	...
	If sItemDisplayName || kItemEnchantment
		;Debug.Trace("vFF/CM: " + kItem + " is enchanted/forged item " + sItemDisplayName)
...
;			kItem.SendModEvent("vFFC_EquipmentSaved","",iHand)
;		Else ;Was not saved from player, indicate this with iHand = -1
;			kItem.SendModEvent("vFFC_EquipmentSaved","",-1)
;		EndIf
;	EndIf
	;Debug.Trace("vFF/CM: Finished serializing " + kItem.GetName() + ", JMap count is " + JMap.Count(jItemInfo))

	Return vFF_API_Item.SaveItem(jItemInfo)
EndFunction
...
{Fills the JMap jEquipmentInfo with all info from Form kItem.}
	...
	Enchantment kItemEnchantment
	If kItem
		;Debug.Trace("vFF/CM: " + kItem.GetName() + " is Mod ID " + (kItem.GetFormID() / 0x1000000))
	...
	EndIf
	;Debug.Trace("vFF/CM: Serializing " + kItem.GetName() + "...")
	If (kItem as Weapon)
		isWeapon = True
		isEnchantable = True
		Int iWeaponType = (kItem as Weapon).GetWeaponType()
...
;		EndIf
;	EndIf
	;Debug.Trace("vFF/CM: Finished serializing " + kItem.GetName() + ", JMap count is " + JMap.Count(jEquipmentInfo))

	Return vFF_API_Item.SaveItem(jEquipmentInfo)
EndFunction
...
{Apply the customization information from ajItemInfo to akObject.}
	...
	If JMap.getInt(jItem,"IsCustom")
		...
		;Debug.Trace("vFF/CM/" + sCharacterName + ":  WornObject.SetItemMaxCharge(kCharacterActor," + iHand + ",0," + JMap.getFlt(jItem,"ItemMaxCharge"))
		kObject.SetItemMaxCharge(JMap.getFlt(jItem,"ItemMaxCharge"))
		If sDisplayName ; Will be blank if player hasn't renamed the item
			;Debug.Trace("vFF/CM/" + sCharacterName + ":  WornObject.SetDisplayName(kCharacterActor," + iHand + ",0," + sDisplayName)
		...
		If JValue.solveInt(jItem,".Enchantment.IsCustom")
			...
			;Debug.Trace("vFF/CM/" + sCharacterName + ":  " + sDisplayName + " has a customized enchantment with " + inumEffects + " magiceffects!")
			Int j = 0
			...
			While j < iNumEffects
				...
				j += 1
			EndWhile
			;Debug.Trace("vFF/CM/" + sCharacterName + ":  " + sDisplayName + " creating custom enchantment...")
			...
			;Debug.Trace("vFF/CM/" + sCharacterName + ":  " + sDisplayName + " done!")
		EndIf
	Else
		kObject.SetItemCharge(JMap.getFlt(jItem,"ItemCharge"))
	EndIf
...
String Function SerializePotion(Form akItem) Global
{
	...
	;Debug.Trace("vFF/CM: Finished serializing " + akItem.GetName() + ", JMap count is " + JMap.Count(jPotionInfo))
	Return vFF_API_Item.SaveItem(jPotionInfo)
EndFunction
...
Function DebugTraceAPIItem(String sDebugString, Int iSeverity = 0) Global
	Debug.Trace("vFF/API/Item: " + sDebugString,iSeverity)
EndFunction
