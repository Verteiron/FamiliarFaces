Scriptname vFFP_TrophyBase extends vFFC_BaseQuest  
...
vFFP_TrophyManager	Property	TrophyManager			Auto Hidden
...
{Create a new Trophy template from akForm, with the specified offset, angle and scale.
 Returns: Index of the new template to be used with other functions.}
	...
	_TemplateObjects[idx] = TrophyBaseObject.PlaceAtMe(vFFP_TrophyObjectBase, abInitiallyDisabled = True)
	vFFP_TrophyObject TrophyObject = _TemplateObjects[idx] as vFFP_TrophyObject
...
Function SendSelfMessage(String asMessage)
	Int iHandle = ModEvent.Create("vFFP_TrophySelfMessage" + TrophyName)
	If iHandle
...
Function RegisterForModEvents()
	If DoNotRegister
		Return
	EndIf
	RegisterForModEvent("vFFP_TrophyManagerReady","OnTrophyManagerReady")
	RegisterForModEvent("vFFP_TrophyCheckAvailable","OnTrophyCheckAvailable")
	RegisterForModEvent("vFFP_TrophySelfMessage" + TrophyName,"OnTrophySelfMessage")
EndFunction
...
Function SendRegisterEvent()
	Int iHandle = ModEvent.Create("vFFP_TrophyRegister")
	If iHandle
		ModEvent.PushString(iHandle,TrophyName)
		ModEvent.PushForm(iHandle,Self)
		ModEvent.Send(iHandle)
	Else
		DebugTrace("WARNING: Couldn't send vFFP_TrophyRegister!",1)
	EndIf
	...
	EndIf
	If !TrophyFadeInFXS
		TrophyFadeInFXS = GetFormFromFile(0x0200a2bd,"vFFP_ShrineOfHeroes.esp") as EffectShader
	EndIf
EndFunction
...
Function SendAvailableEvent(Int aiAvailable = 0)
	Int iHandle = ModEvent.Create("vFFP_TrophyAvailable")
	If iHandle
		ModEvent.PushString(iHandle,TrophyName)
		ModEvent.PushInt(iHandle,aiAvailable)
		ModEvent.Send(iHandle)
	Else
		DebugTrace("WARNING: Couldn't send vFFP_TrophyAvailable!",1)
	EndIf
EndFunction
...
Function _CreateTemplates()
	If !TrophyBaseObject
		TrophyBaseObject = TrophyOrigin.PlaceAtMe(vFFP_TrophyEmptyBase, abInitiallyDisabled = True)
	...
	While i < iLen && _TemplatesToDisplay[i]
		...
		If _TemplateObjects[idx]
			...
			;If _TemplateObjects[idx].GetBaseObject()
				;DebugTrace("Registering template " + idx + " - " + _TemplateObjects[idx] + " for event vFFP_TrophyDisplay" + TrophyName + sTargetFormID + "!")
				(_TemplateObjects[idx] as vFFP_TrophyObject).TrophyIndex = idx
				_TemplateObjects[idx].RegisterForModEvent("vFFP_TrophyDisplay" + TrophyName + sTargetFormID,"OnTrophyDisplay")	
			;EndIf
		EndIf
		i += 1
	EndWhile
...
Function SendDisplayEvent(ObjectReference akTarget)
	String sTargetFormID = GetFormIDString(akTarget)
	Int iHandle = ModEvent.Create("vFFP_TrophyDisplay" + TrophyName + sTargetFormID)
	If iHandle
		ModEvent.PushForm(iHandle,akTarget)
		ModEvent.PushBool(iHandle,False)
		ModEvent.Send(iHandle)
	Else
		DebugTrace("WARNING, couldn't send vFFP_TrophyDisplay" + TrophyName + sTargetFormID + " event!",1)
	EndIf
EndFunction
...
Function _DisplayObject(ObjectReference akTarget, ObjectReference akTemplate)
	...
	If akTemplate as vFFP_TrophyObject
		(akTemplate as vFFP_TrophyObject).PlaceTrophyForm(akTarget)
		Return
	EndIf
...
ObjectReference Function _GetBannerTemplate(ObjectReference akTarget, String asBannerType = "Standing")
	...
	Keyword kBannerKeyword = Keyword.GetKeyword("vFFP_Banner" + asBannerType + iBannerIndex)
