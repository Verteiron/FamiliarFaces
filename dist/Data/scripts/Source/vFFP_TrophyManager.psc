Scriptname vFFP_TrophyManager extends vFFC_ManagerBase
{Handles registration and tracking of trophies.}

;=== [ vFFP_TrophyManager.psc ] ===========================================---
...
Static				Property vFFP_AlcoveTrophyPlacementMarker		Auto

Form				Property vFFP_TrophyBannerHangingMarker			Auto
Form				Property vFFP_TrophyBannerStandingMarker		Auto

FormList			Property vFFP_BannerAnchors						Auto
...
{Sent once initialization is done to let TrophyBases know they can register themselves.}
	...
	RegisterForModEvent("vFFP_TrophyRegister","OnTrophyRegister")
	Int iHandle = ModEvent.Create("vFFP_TrophyManagerReady")
	If iHandle
		ModEvent.PushForm(iHandle,Self)
		ModEvent.Send(iHandle)
	Else
		DebugTrace("WARNING! Could not send vFFP_TrophyRegister event!",1)
	EndIf
EndFunction
...
{Place all Trophies earned by sCharacterID around akTargetObject. Trophies will not be displayed until DisplayTrophies is called.
 Returns: Number of trophies processed.}
	...
	JValue.WriteToFile(jCharacterTrophies,JContainers.userDirectory() + "vFF/displaytrophies.json")
		...
		While i < iCount
			...
			If kTrophyObject
				If kTrophyObject as vFFP_TrophyObject
					(kTrophyObject as vFFP_TrophyObject).DeleteTrophyForm()
				EndIf
...
{Update the Trophies available for the current player character. Relies on vFFP_TrophyBase@_IsAvailable.}
	...
	While i > 0
		...
		String sTrophyName = JArray.GetStr(jTrophyNames,i)
		If sTrophyName
			vFFP_TrophyBase kTrophy = GetRegForm("Trophies." + sTrophyName + ".Form") as vFFP_TrophyBase
			If kTrophy
...
{Returns: Hanging banner base form.}
	Return vFFP_TrophyBannerHangingMarker
EndFunction

Form Function GetStandingBannerMarker()
{Returns: Standing banner base form.}
	Return vFFP_TrophyBannerStandingMarker
EndFunction
